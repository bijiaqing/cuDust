...
void ssa_substep_2 (swarm *dev_particle, real *dev_optdepth)
{
    ...
    if(idx >= 0 && idx < NUM_PAR)
    {
        ...
        real v_rad_i, rad_1, v_rad_1, vg_rad_1, forc_rad_1, forc_rad_2, rad_j, v_rad_j, par_rad;
        real l_col_i, col_1, l_col_1, lg_col_1, torq_col_1, torq_col_2, col_j, l_col_j, par_col;
        real optdepth, ts_1, tau_1, eta_1, beta_1;
        ...
        ts_1  = ST_REF / sqrt(G*M_STAR / RAD_REF / RAD_REF / RAD_REF);
        // correct for radial gas density and sound speed
        ts_1 *= pow(bigR_1 / RAD_REF, - 0.5*IDX_TEMP - IDX_SIGG + 1.0);
        // correct for vertical gas density
        ts_1 *= exp(bigZ_1*bigZ_1 / (2.0*ASP_REF*ASP_REF*bigR_1*bigR_1*pow(bigR_1 / RAD_REF, IDX_TEMP + 1.0)));
        tau_1 = dt / ts_1;
        ...
        beta_1   = 1.0 - BETA_REF*exp(-optdepth);
